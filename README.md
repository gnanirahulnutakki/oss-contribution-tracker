...
| [akuity/kargo#6685](https://github.com/akuity/kargo/issues/6685)<br>Webhooks server &#40;kubernetes-webhooks-server&#41; is missing probe support that all other servers have | assignee | **Assigned** | assigned to @gnanirahulnutakki | A private, uncommitted implementation is fully qualified and now replayed byte-identically onto current main d30e014; the upstream deletion of an accidental 664-line design document is incorporated and will not be reintroduced. Focused race tests, all 151 chart assertions, chart lint, generated docs, and diff checks pass on that exact base. The exact built image sha256:eaab195d887f previously reached Ready in disposable kind v1.34: /healthz reported ping, /readyz reported webhook-server, both returned HTTP 200, restart count stayed zero, logs had no TLS-probe noise, read-only Freight/Stage cache RBAC was sufficient, and all 11 rolling-restart samples retained a ready Service endpoint. The cluster, TLS material, and image were removed. Keep it unpublished at 6/4. A new authored PR is permitted only when the pre-publication count is 3 or fewer, preserving the hard cap of 4; then recheck live assignment, competing work, main, and contribution policy before committing. |
...
Last public state change recorded: **2026-08-13T02:04:45+00:00**.
